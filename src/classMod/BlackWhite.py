...
class BlackAndWhite(View):
    def __init__(self, file_name):
        super().__init__()
        self.file_name = file_name

    @button(label='Transform! ✨', style=ButtonStyle.blurple)
    async def b_and_w(self, interaction: Interaction, button: Button):
        link_view = Link()
        embed = Embed(
        ...
        image_tag = CloudinaryImage(f"Bot/{self.file_name}.png").build_url(effect="grayscale")
        
        embed.set_image(url=image_tag)
        
        link_view.add_item(Button(label='Download ✨', style=ButtonStyle.url, url=image_tag, emoji="<a:vibing:747680206734622740>"))
